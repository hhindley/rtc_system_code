...
plot(sol.t, rm_a, linecolor=:green3, annotations=([1.5e8],[rm_a[end]+72], "mRNA-RtcA/B"), annotationcolor=:green3, annotationfontsize=10, xaxis=(:log10, (1,Inf)), yaxis=(:log10, (1,Inf)), title="Model solution", legend=false, xlabel="time", ylabel="amount of species")
plot!(sol.t, rtca, linecolor=:green, annotations=([4.2e8],[rtca[end]+10000], "RtcA/B"), annotationcolor=:green, annotationfontsize=10)
plot!(sol.t, rm_r, linecolor=:orange, annotations=([2e8],[rm_r[end]+8], "mRNA-RtcR"), annotationcolor=:orange, annotationfontsize=10)
plot!(sol.t, rtcr, linecolor=:darkorange2, annotations=([4.8e8],[rtcr[end]+1000], "RtcR"), annotationcolor=:darkorange2, annotationfontsize=10)
plot!(sol.t, ribo_h, linecolor=:dodgerblue2, annotations=([6.2e8],[ribo_h[end]+3], "Rh"), annotationcolor=:dodgerblue2, annotationfontsize=10)

plot(sol.t, ribo_h, linecolor=:dodgerblue2, annotations=([6e8],[ribo_h[end]+0.3], "Rh"), annotationcolor=:dodgerblue2, annotationfontsize=10, xaxis=(:log10, (1,Inf)), title="Model solution - ribosomes", legend=false, xlabel="time", ylabel="amount of species")
plot!(sol.t, ribo_d, linecolor=:blue, annotations=([6e8],[ribo_d[end]+1], "Rd"), annotationcolor=:blue, annotationfontsize=10)
plot!(sol.t, ribo_t, linecolor=:deepskyblue, annotations=([6e8],[ribo_t[end]+0.3], "Rt"), annotationcolor=:deepskyblue, annotationfontsize=10)
...
atp_range_long = range(0,20, length=100)
include("fullmodel_params.jl")
...
function plot_inset()
    p = plot(atp_range_long, [rtca_sols], title="Dependence on ATP", xlabel="ATP", ylabel="amount of species at steady state", legend=false, linecolor=:green, annotations=([atp_range_long[end]-2],[rtca_sols[end]], "RtcA/B"), annotationcolor=:green)
    plot!(atp_range_long, rtcr_sols, linecolor=:darkorange2, annotations=([atp_range_long[end]],[rtcr_sols[end]+5000], "RtcR"), annotationcolor=:darkorange2)
    lens!([0,5],[0,1000], inset = (1, bbox(0.2, 0.1, 0.3, 0.3)), lc=:darkgrey)
...
include("fullmodel_params.jl")
kr_plot = plot_long_range_results(atp_range_long, params_new, 1, RTC_SYSTEM2!, init_rtc, params_new, species_rtc, "kr")

L_range_long = range(0,100, length=20)
include("fullmodel_params.jl")
L_plot = plot_long_range_results(atp_range_long, params_new, 2, RTC_SYSTEM2!, init_rtc, params_new, species_rtc, "L")

c_range_long = range(0,100, length=20)
include("fullmodel_params.jl")
c_plot = plot_long_range_results(atp_range_long, params_new, 3, RTC_SYSTEM2!, init_rtc, params_new, species_rtc, "c")

sigma_range_long = range(0,10, length=20)
include("fullmodel_params.jl")
sigma_plot = plot_long_range_results(atp_range_long, params_new, 4, RTC_SYSTEM2!, init_rtc, params_new, species_rtc, "sigma")

k1_range_long = range(0,1, length=20)
include("fullmodel_params.jl")
k1_plot = plot_long_range_results(atp_range_long, params_new, 11, RTC_SYSTEM2!, init_rtc, params_new, species_rtc, "k1")

k2_range_long = range(0,1, length=20)
include("fullmodel_params.jl")
k2_plot = plot_long_range_results(atp_range_long, params_new, 12, RTC_SYSTEM2!, init_rtc, params_new, species_rtc, "k2")

k3_range_long = range(0,1, length=20)
include("fullmodel_params.jl")
k3_plot = plot_long_range_results(atp_range_long, params_new, 13, RTC_SYSTEM2!, init_rtc, params_new, species_rtc, "k3")

k4_range_long = range(0,1, length=20)
include("fullmodel_params.jl")
k4_plot = plot_long_range_results(atp_range_long, params_new, 14, RTC_SYSTEM2!, init_rtc, params_new, species_rtc, "k4")

k5_range_long = range(0,1, length=20)
include("fullmodel_params.jl")
k5_plot = plot_long_range_results(atp_range_long, params_new, 15, RTC_SYSTEM2!, init_rtc, params_new, species_rtc, "k5")

wrtc_range_long = range(0,10, length=20)
include("fullmodel_params.jl")
wrtc_plot = plot_long_range_results(atp_range_long, params_new, 6, RTC_SYSTEM2!, init_rtc, params_new, species_rtc, "wrtc")

theta_range_long = range(0,20, length=20)
include("fullmodel_params.jl")
theta_plot = plot_long_range_results(atp_range_long, params_new, 7, RTC_SYSTEM2!, init_rtc, params_new, species_rtc, "theta")

max_range_long = range(0,10, length=20)
include("fullmodel_params.jl")
max_plot = plot_long_range_results(atp_range_long, params_new, 8, RTC_SYSTEM2!, init_rtc, params_new, species_rtc, "max")

thr_range_long = range(0,10, length=20)
include("fullmodel_params.jl")
thr_plot = plot_long_range_results(atp_range_long, params_new, 9, RTC_SYSTEM2!, init_rtc, params_new, species_rtc, "thr")

k1a_range_long = range(0,2, length=20)
include("fullmodel_params.jl")
k1a_plot = plot_long_range_results(atp_range_long, params_new, 16, RTC_SYSTEM2!, init_rtc, params_new, species_rtc, "k1a")

k2a_range_long = range(0,2, length=20)
include("fullmodel_params.jl")
k2a_plot = plot_long_range_results(atp_range_long, params_new, 17, RTC_SYSTEM2!, init_rtc, params_new, species_rtc, "k2a")

k3a_range_long = range(0,1, length=20)
include("fullmodel_params.jl")
k3a_plot = plot_long_range_results(atp_range_long, params_new, 18, RTC_SYSTEM2!, init_rtc, params_new, species_rtc, "k3a")

kab_range_long = range(0,2, length=20)
include("fullmodel_params.jl")
kab_plot = plot_long_range_results(atp_range_long, params_new, 19, RTC_SYSTEM2!, init_rtc, params_new, species_rtc, "kab")

kbb_range_long = range(0,2, length=20)
include("fullmodel_params.jl")
kbb_plot = plot_long_range_results(atp_range_long, params_new, 20, RTC_SYSTEM2!, init_rtc, params_new, species_rtc, "kbb")

kcb_range_long = range(0,1, length=20)
include("fullmodel_params.jl")
kcb_plot = plot_long_range_results(atp_range_long, params_new, 21, RTC_SYSTEM2!, init_rtc, params_new, species_rtc, "kcb")
...
rd_range_long = range(0,10, length=20)
include("fullmodel_params.jl")
rd_plot = plot_long_range_results(atp_range_long, params_new, 5, RTC_SYSTEM2!, init_rtc, params_new, species_rtc, "ATP")

rh_range_long = range(0,10, length=50)
include("fullmodel_params.jl")
rh_plot = plot_long_range_results(atp_range_long, params_new, 5, RTC_SYSTEM2!, init_rtc, params_new, species_rtc, "ATP")

rt_range_long = range(0,10, length=100)
include("fullmodel_params.jl")
rt_plot = plot_long_range_results(atp_range_long, params_new, 5, RTC_SYSTEM2!, init_rtc, params_new, species_rtc, "ATP")
